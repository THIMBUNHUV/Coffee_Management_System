...
class ProfileScreen extends StatelessWidget {
    ...
    return Stack(
      children: [
        Container(
          ...
          child: authProvider.customerImagePath != null
              ? FutureBuilder<File?>(
                  future: authProvider.getCustomerImageFile(),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow(
            ...
            label: 'Full Name',
            value: authProvider.customer!.name,
          ),
          const SizedBox(height: 12),
          _buildInfoRow(
            icon: Icons.email_outlined,
            label: 'Email Address',
            value: authProvider.customer!.email,
          ),
          const SizedBox(height: 12),
          _buildInfoRow(
            icon: Icons.phone_iphone,
            label: 'Phone Number',
            value: authProvider.customer!.phone ?? 'Not provided',
            isMissing: authProvider.customer!.phone == null,
          ),
          const SizedBox(height: 12),
          _buildInfoRow(
            icon: Icons.location_on_outlined,
            label: 'Address',
            value: authProvider.customer!.address ?? 'Not provided',
            isMissing: authProvider.customer!.address == null,
          ),
        ],
      ),
    );
  ...
  Future<void> _pickImageFromGallery(AuthProvider authProvider) async {
    ...
    if (pickedFile != null) {
      await authProvider.saveCustomerImage(File(pickedFile.path));
    }
  }
  ...
  void _showEditProfileDialog(BuildContext context, AuthProvider authProvider) {
    final nameController = TextEditingController(
      text: authProvider.customer?.name,
    );
    final emailController = TextEditingController(
      text: authProvider.customer?.email,
    );
    final passwordController = TextEditingController(
      text: authProvider.customer?.password,
    );
    final phoneController = TextEditingController(
      text: authProvider.customer?.phone ?? '',
    );
    final addressController = TextEditingController(
      text: authProvider.customer?.address ?? '',
    );
                            ...
                            onPressed: () async {
                              ...
                              try {
                                ...
                                final updatedCustomer = Customer(
                                  id: authProvider.customer!.id,
                                  name: nameController.text,
                                  email: emailController.text,
                                  password: authProvider.customer!.password,
                                ...
                                // Update in database
                                await authProvider.updateCustomerProfile(
                                  updatedCustomer,
                                );
